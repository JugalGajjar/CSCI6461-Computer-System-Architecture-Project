...
public class Simulator {
    ...
    public Simulator() {
        ...
        cc.gridx = 0;
        cc.gridy = 0;
        JButton startProgram1Button = new JButton("Start Program 1");
        startProgram1Button.setPreferredSize(new Dimension(135, 30));
        startProgram1Button.addActionListener(e -> startProgram1());
        convertPanel.add(startProgram1Button, cc);

        cc.gridy = 1;
        JButton findClosestButton = new JButton("Find Closest Number");
        findClosestButton.setPreferredSize(new Dimension(80, 30));
        findClosestButton.addActionListener(e -> findClosest());
        cc.gridwidth = 2;
        convertPanel.add(findClosestButton, cc);
        cc.gridwidth = 1;
    ...
    private void findClosest() {
        if (inputTextArea.getText().isEmpty()) {
            JOptionPane.showMessageDialog(null, "Enter a number to find closest value.");
        } else {
            String numbers = inputTextArea.getText();
            if (numbers.contains(",") || numbers.contains(" ")) {
                JOptionPane.showMessageDialog(null, "Enter a number to find closest value.");
            } else {
                inputTextArea.setText("");
                intArray[20] = Integer.parseInt(numbers);

                readAndConvertInputFile("program1.txt");
                readUsingFileName("./load.txt");

                // Find the closest number to the target
                int closestNumber = intArray[0];
                int target = intArray[20];
                int minDifference = Math.abs(intArray[0] - target);

                for (int i = 1; i < 20; i++) {
                    int currentDifference = Math.abs(intArray[i] - target);
                    if (currentDifference < minDifference) {
                        minDifference = currentDifference;
                        closestNumber = intArray[i];
                    }
                }
                outputTextArea.setText("The closest number to " + target + " is: " + closestNumber);
            }
        }
    }
    ...
    private void startProgram1() {
        if (inputTextArea.getText().isEmpty()) {
            JOptionPane.showMessageDialog(null, "Enter 20 numbers separated by commas.");
        } else {
            String[] numbers = inputTextArea.getText().split(",");
    ...
    private void readAndConvertInputFile(String name) {
        ...
        if (hasAlphabets) {
            if (intArray.length >= 21) {
                // Replace placeholders in each line
                for (int i = 0; i < cleanInstructions.length; i++) {
                    String line = cleanInstructions[i];
                    StringBuilder modifiedLine = new StringBuilder();
                    int startIndex = 0;
                    while (startIndex < line.length()) {
                        int endIndex = line.indexOf("<", startIndex);
                        if (endIndex == -1) {
                            modifiedLine.append(line.substring(startIndex));
                            break;
                        }
                        modifiedLine.append(line.substring(startIndex, endIndex));
                        endIndex = line.indexOf(">", endIndex);
                        if (endIndex == -1) {
                            // Handle invalid placeholder format
                            System.out.println("Invalid placeholder format: " + line.substring(startIndex));
                            break;
                        }
                        int index = Integer.parseInt(line.substring(endIndex - 1, endIndex));
                        modifiedLine.append(intArray[index]);
                        startIndex = endIndex + 1;
                    }
                    cleanInstructions[i] = modifiedLine.toString();
                }
            }
            ...
            // Create listing file
            fileHandler.createListingFile(instructions, keys, values, "listing.txt");
